...
const heroSlides = [
  {
    ...
    subtitle:
      "Our officers are perfectly adapted to circumstances with your trusted shield and professional excellence ensuring maximum security coverage.",
  ...
  },
  {
    ...
    subtitle:
      "Disciplined ex-servicemen personnel providing comprehensive security solutions with protection without pause and unwavering commitment to safety.",
  ...
  },
  {
    ...
    features: ["25⁺ Years of Experience", "Trained Professionals", "Northeast Coverage"],
  },
  {
    image: "/IMG-20201127-WA0009.jpg",
    title: "Advanced Security Solutions",
    subtitle:
      "State-of-the-art security infrastructure combined with highly trained personnel ensuring comprehensive protection for your assets.",
...
export default function Hero() {
  ...
  // Two-layer crossfade between images
  const doCrossfade = () => {
    ...
    tl.to(".hero-image-prev", { opacity: 0, duration: 0.5, ease: "power2.in" }).to(".hero-image-current", { opacity: 1, duration: 0.5, ease: "power2.out" }, "<")
  }
  ...
  const smoothChangeSlide = (index: number) => {
    ...
    // Start crossfade once the next image is loaded
  ...
  // Visitor counter: GA4 realtime active users
  useEffect(() => {
    async function fetchCount() {
      try {
        const res = await fetch("/api/ga/realtime", { cache: "no-store" })
        const data = await res.json()
        let base = Number(data.activeUsers ?? 0)
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[70vh]">
          ...
          <div className="order-1 lg:order-2 text-center lg:text-left">
            <div className="hero-content space-y-8">
              <h1 className="hero-title text-4xl lg:text-6xl xl:text-7xl leading-tight tracking-wider">{currentSlideData.title}</h1>

              <p className="hero-subtitle text-lg lg:text-xl xl:text-2xl max-w-2xl mx-auto lg:mx-0">{currentSlideData.subtitle}</p>

              <div className="hero-features flex flex-wrap gap-3 justify-center lg:justify-start">
                {currentSlideData.features.map((feature, index) => (
                  <span key={index} className="px-4 py-2 rounded-full text-sm font-[var(--font-inter)] font-medium backdrop-blur-sm">
                    {feature}
                  </span>
                ))}
              </div>
              ...
              <div className="hero-stats grid grid-cols-1 sm:grid-cols-3 gap-6 pt-8">
                <div className="text-center">
                  ...
                  <div className="text-sm text-[#2f2f2f] font-[var(--font-inter)]">Live Visitors</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl lg:text-3xl font-[var(--font-roboto-slab)] font-bold text-[#556B2F] mb-2">24/7</div>
                  <div className="text-sm text-[#2f2f2f] font-[var(--font-inter)]">Operations</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl lg:text-3xl font-[var(--font-roboto-slab)] font-bold text-[#8b0000] mb-2">ISO 9001:2015</div>
                  <div className="text-sm text-[#2f2f2f] font-[var(--font-inter)]">Certified</div>
                </div>
              </div>
            </div>
          </div>
